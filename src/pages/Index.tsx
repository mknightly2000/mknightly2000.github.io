...
import { Github, Linkedin, FileDown, ArrowRight, Mail, FileText } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-3xl px-6 py-24 text-center sm:py-32">
          ...
          <h1 className="text-balance mt-3 text-5xl font-semibold tracking-tight sm:text-7xl">
          ...
          <p className="mt-4 font-mono text-sm text-muted-foreground sm:text-base">
            ...
            <span className="text-foreground/80">ai researcher</span>
          </p>
          <p className="text-balance mx-auto mt-8 max-w-xl text-base leading-relaxed text-muted-foreground sm:text-lg">
            I build thoughtful tools at the intersection of design and machine learning —
            from native iOS apps to retrieval systems for production LLMs.
          </p>
      ...
      {/* Experience */}
      <section id="experience" className="border-t border-border/60 bg-card/30">
            ...
            <ol className="relative space-y-2 border-l border-border pl-6">
              {workHistory.map((w) => (
                <li key={w.slug} className="relative">
                  ...
                  >
                    <span className="absolute -left-[31px] top-6 h-2 w-2 rounded-full bg-border-strong transition-colors group-hover:bg-primary ml-[2.5px]" />
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      <h3 className="text-base font-medium text-foreground">
                        {w.role}{" "}
                        <span className="text-muted-foreground">· {w.company}</span>
                      </h3>
                      <span className="font-mono text-xs text-muted-foreground">{w.period}</span>
                    </div>
              ...
              {education.map((e) => (
                <li key={e.slug} className="relative">
                  <Link
                    ...
                    className="group block rounded-xl p-4 transition-colors hover:bg-secondary"
                  >
                    <span className="absolute -left-[31px] top-6 h-2 w-2 rounded-full bg-border-strong transition-colors group-hover:bg-primary mx-[2.5px]" />
                    <div className="flex flex-wrap items-baseline justify-between gap-2">
                      <h3 className="text-base font-medium text-foreground">
                        {e.degree}{" "}
      ...
      <footer className="border-t border-border/60">
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-4 px-6 py-10 sm:flex-row sm:items-center">
          <div className="font-mono text-xs text-muted-foreground">
            © {new Date().getFullYear()} Michael Knightly. Built with care.
          </div>
          <a
            href="mailto:hello@example.com"
